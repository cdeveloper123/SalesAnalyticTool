...
export function evaluateDeal(input, productData, pricingData) {
  ...
  return {
    ...
    dealScore: {
      ...
      weighted: {
        marginContribution: Number((scores.netMarginScore * WEIGHTS.netMargin).toFixed(2)),
        demandContribution: Number((scores.demandConfidenceScore * WEIGHTS.demandConfidence).toFixed(2)),
        volumeContribution: Number((scores.volumeRiskScore * WEIGHTS.volumeRisk).toFixed(2)),
        reliabilityContribution: Number((scores.dataReliabilityScore * WEIGHTS.dataReliability).toFixed(2))
      },
      weights: {
